...
Pass `-i` / `--interactive` to answer staged prompts on a TTY instead of writing a config file: sections already configured in the file (non-empty `images`, `registries`, `tags`, `platforms`) are reused silently without re-prompting, missing sections are prompted with config values as defaults, registry auth that resolves from the environment skips its prompts, explicit CLI flags still win, and `--dry-run` shapes the plan without auth prompts or confirmation. Registry hostnames are picked from a common-registry list with a custom-hostname entry last. Without a TTY, `-i` fails closed before any Docker invocation. Prompted registry passwords are masked, held in memory for this run's login only, and never saved to disk.
...
Registry credentials come from the config `auth` map (`{ "<hostname>": { "username", "passwordEnv" } }`, or `usernameEnv` instead of `username`) and travel to `docker login` via password-stdin only — the password always comes from the named environment variable, while the username may be a literal string. Registries without an `auth` entry are pushed without a login step. Secrets never appear in argv, summaries, or error output.
